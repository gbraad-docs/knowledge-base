# Containers & Cloud

Notes on container runtimes, orchestration, and cloud platforms.

- [Containers](containers.md) - container runtimes and fundamentals
- [Docker](docker.md) - Docker CLI, Compose, and Containerfiles
- [Atomic](atomic.md) - Project Atomic / rpm-ostree based systems
- [cloud-init](cloudinit.md) - cloud instance initialization
- [Cloud Foundry](cloudfoundry.md) - PaaS platform
- [Ceph](ceph.md) - distributed storage
- [ManageIQ](manageiq.md) - cloud management platform
- [Kubernetes](kubernetes/) - container orchestration
- [OpenShift](openshift/) - Kubernetes distribution by Red Hat
- [OpenStack](openstack/) - open source cloud infrastructure
